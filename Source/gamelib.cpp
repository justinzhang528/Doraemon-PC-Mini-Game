...
void CGameState::ShowInitProgress(int percent)
{
	...
	loading.LoadBitmap(".\\bitmaps\\showprogress.bmp");
	...
	CBrush b2(RGB(8,173,239));					// 畫黃色 progrss進度
	pDC->SelectObject(&b2);
...
void CGameState::AddScore(int n)
{
	score += n;
}

void CGameState::ReduceEnemyCount()
{
	enemyCount--;
}
...
: NUM_GAME_STATES(4)
{
	...
	gameStateTable[GAME_STATE_REST] = new CGameStateRest(this);
	gameState = NULL;
}
...
void CGame::OnDraw()
{
	...
	if (!running) {
		...
		//
		CMovingBitmap bmp;
		bmp.LoadBitmap(".\\bitmaps\\pause.bmp");
		bmp.SetTopLeft(0,0);
		bmp.ShowBitmap();
	}
	CDDraw::BltBackToPrimary();				// 將 Back Plain 貼到螢幕
...
int CGameState::MapLevel = 0;
int CGameState::enemyCount = 0;
int CGameState::score = 0;
int CGameState::highestScore = 0;
int CGameState::left = 0;
